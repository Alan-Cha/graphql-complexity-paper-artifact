...
function getOperationType(opNode, schema) {
    switch (opNode.operation) {
        ...
        default:
            throw new Error("Cannot analyze response for operation \"" + opNode.operation + "\"");
    }
}
        ...
        return graphql_1.visit(ast, {
            ...
            Field: {
                enter: function (node) {
                    ...
                    var parentType = state_1.visitedTypes[state_1.visitedTypes.length - 1];
                            ...
                            if (arg) {
                                if (graphql_1.isEnumType(graphql_1.getNamedType(arg.type))) {
                                    ...
                                    switch (typeof variableValue) {
                                        ...
                                        default:
                                            throw new Error("Could not handle variable value type '" + typeof variableValue + "'");
                                    }
                                }
                            }
                        }
                        else {
                            throw new Error("Could not find variable key '" + variableKey + "' in variables '" + variables + "'");
                        }
                    }
                },
            },
